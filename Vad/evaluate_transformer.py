...
def evaluate(test_dataset_args, test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256,
    ...
    # test
    generator.eval()
    for k, (feats, objects, bboxes) in enumerate(tqdm(test_dataloader, desc='test', leave=False)):
        ...
        if objects[0].shape[0] == 0:
            psnr_list[sorted(videos.keys())[video_num]].append(utils.psnr(0.0000000001))
            frame_index += 1
            continue
        ...
        target = objects[0, -1, :]
        outputs = generator(feats[:, (test_dataset_args['t_length'] - 1) // 2], bboxes.squeeze(0))  # 取中间帧的特征

        # print(outputs.data.shape)
        # print(target.data.shape)
        object_num = outputs.shape[0]
        max_mse = -1

        for i in range(object_num):
            mse = int_loss((outputs[i] + 1) / 2, (target[i] + 1) / 2).item()
            # print(mse)
        ...
        #     plt.subplot(1, X_bbox.shape[0], index)
        #     index += 1
        #
        #     img = X_bbox[i, :, :, :].cpu().clone()
        #
        #     img = unloader(img)
        #     img = np.array(img)
        ...
        #     plt.imshow(img)
        #
        # plt.savefig('object_out' + '.jpg')
        #

        psnr_list[sorted(videos.keys())[video_num] ].append(utils.psnr(max_mse))
