...
from models.vadTransformer import VadTransformer
from models.unet import UNet
from models.networks import define_G
from models.pix2pix_networks import PixelDiscriminator
# from liteFlownet.lite_flownet import Network, batch_estimate
from losses import *
from vad_dataloader_transformer import VadDataset

import torchvision.transforms as transforms
from evaluate_transformer import *
...
def train(config):
    ...
    config_fastercnn_file = "../maskrcnn-benchmark-master/configs/caffe2/e2e_mask_rcnn_R_50_C4_1x_caffe2.yaml"
    # config_file = "../maskrcnn-benchmark-master/configs/caffe2/e2e_faster_rcnn_R_101_FPN_1x_caffe2.yaml"
    # update the config options with the config file
    cfg.merge_from_file(config_fastercnn_file)

    train_dataset = VadDataset(fastrcnn_config=cfg,video_folder= train_folder, bbox_folder = None, flow_folder=None,
    ...
    test_dataset = VadDataset(fastrcnn_config=cfg,video_folder= test_folder, bbox_folder = None, flow_folder=None,
                            transform=transforms.Compose([transforms.ToTensor()]),
                            resize_height=train_dataset_args['h'], resize_width=train_dataset_args['w'],
                               dataset=config['train_dataset_type'], time_step=train_dataset_args['t_length'] - 1,
                               device=device)
    ...
    elif config['generator'] == 'vadTransformer':
        model = VadTransformer(cfg, train_dataset_args['h'], train_dataset_args['w'], train_dataset_args['c'])
    else:
    ...
    for epoch in range(config['epochs']):
        ...
        model.train()
        for j, (feats, objects, bboxes) in enumerate(tqdm(train_dataloader, desc='train', leave=False)):
            # frames = frames.cuda()
            if objects[0].shape[0] == 0:
                continue
            ...
            outputs = model(feats[:, (train_dataset_args['t_length'] - 1) // 2], bboxes.squeeze(0)) # 取中间帧的特征
        ...
        if epoch % save_epoch == 0 or epoch == config['epochs'] - 1:
            ...
            # TODO 
            frame_AUC = evaluate(test_dataset_args, test_dataloader, model, labels_list, videos, dataset=config['test_dataset_type'],device = device,
            ...
            frame_AUC = evaluate(test_dataloader, model, labels_list, videos, dataset=config['test_dataset_type'],device=device,
                frame_height = train_dataset_args['h'], frame_width=train_dataset_args['w']) 
