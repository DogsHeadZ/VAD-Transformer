...
def get_motion_roi(img_batch, bboxes, dataset_name):
    ...
    for bbox in bboxes:
        extend_x1 = int(max(0, bbox[0] - extend))
        extend_y1 = int(max(0, bbox[1] - extend))
        extend_x2 = int(min(bbox[2] + extend, sum_grad.shape[1]))
        extend_y2 = int(min(bbox[3] + extend, sum_grad.shape[0]))
...
def RoIResize(frameName, dataset, model, resize_height, resize_width):
    frame = cv2.imread(frameName)
    frame = cv2.resize(frame, (resize_width, resize_height))
    frame = np.array(frame[:, :, [2, 1, 0]])
    image, feats, appear_boxes = model.run_on_opencv_image(frame)
    appear_boxes = delCoverBboxes(appear_boxes, dataset)
    appear_boxes = [[int(a1) for a1 in a2] for a2 in appear_boxes]

    return appear_boxes, feats


def RoI(frameName, dataset, model):
    frame = cv2.imread(frameName)
    frame = np.array(frame[:, :, [2, 1, 0]])
    image, feats, appear_boxes = model.run_on_opencv_image(frame)
    appear_boxes = delCoverBboxes(appear_boxes, dataset)
...
def RoIAll(frameNames, dataset, model):
    frame = cv2.imread(frameNames[int(len(frameNames) / 2)])
    frame = np.array(frame[:, :, [2, 1, 0]])
    image, feats, appear_boxes = model.run_on_opencv_image(frame)
    appear_boxes = delCoverBboxes(appear_boxes, dataset)

    motion_boxes = get_motion_roi(frameNames, appear_boxes, dataset)
    bboxes = delCoverBboxes(appear_boxes + motion_boxes, dataset)
    bboxes = [[int(a1) for a1 in a2] for a2 in bboxes]
    return bboxes
...
if __name__ == "__main__":
    ...
    frame_path = "../../AllDatasets/avenue/training/frames/"  # frame path
    save_path = "./bboxes/avenue_bf/train/"
    ...
    for clip in clips:
        ...
        for index in range(0, len(filenames)):
            img = os.path.join(path, filenames[index])

            roi, feats = RoI(img, "avenue", coco_demo)
            roi = [[int(a1) for a1 in a2] for a2 in roi]
            record = {'box': roi, 'feats': feats}
            clips_roi.append(record)
            # 可视化
            # img = cv2.imread(img)
